...
class Background(WithExitStackMixin):
    ...
    def make(self):
        ...
        def increase_contrast(channel):
            min_val = auto.np.min(channel)
            max_val = auto.np.max(channel)
            contrast_stretched = (channel - min_val) * (max_val*1.3 - min_val)
            return contrast_stretched
        def adjust_contrast(channel, factor):
            # Apply contrast formula with adjustable factor
            return np.clip(0.5 + factor * (channel - 0.5), 0.0, 1.0)
        
        vertex__color['r'] = increase_contrast(vertex__color['r'])
        vertex__color['g'] = increase_contrast(vertex__color['g'])
        vertex__color['b'] = increase_contrast(vertex__color['b'])

        vertex__color['r'] = adjust_contrast(vertex__color['r'], 1.2)
        vertex__color['g'] = adjust_contrast(vertex__color['g'], 1.2)
        vertex__color['b'] = adjust_contrast(vertex__color['b'], 1.2)
        ...
        lib.ospSetFloat(material, b'ns', 0.7)
...
class City(WithExitStackMixin):
    ...
    def make(self):
        ...
        earth = auto.pathlib.Path('data') / 'TEarth/' / 'Earth'
        ...
        usa = auto.pathlib.Path('data') / 'TEarth' / 'USA'
