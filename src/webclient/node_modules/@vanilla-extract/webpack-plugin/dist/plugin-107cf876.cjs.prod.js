...
const escapeWebpackTemplateString = s => s.replaceAll(templateStringRegexp, '[\\$1\\]');
    ...
    if (compat.isWebpack5) {
      childCompiler.hooks.compilation.tap(compilerName, compilation => {
        compilation.hooks.processAssets.tap(compilerName, () => {
          source = compilation.assets[loader.resourcePath] && compilation.assets[loader.resourcePath].source();
...
function markCSSFilesAsSideEffects(compiler, compat) {
  compiler.hooks.normalModuleFactory.tap(pluginName, nmf => {
    if (compat.isWebpack5) {
      ...
      createData => {
        if (createData.matchResource && (createData.matchResource.endsWith('.vanilla.css') || createData.matchResource.endsWith('vanilla.virtual.css'))) {
      ...
      result => {
        if (result.matchResource && (result.matchResource.endsWith('.vanilla.css') || result.matchResource.endsWith('vanilla.virtual.css'))) {
...
class AbstractVanillaExtractPlugin {
  constructor(options = {}) {
    const {
  ...
  inject(compiler, virtualLoader) {
    ...
    (_compiler$options$mod = compiler.options.module) === null || _compiler$options$mod === void 0 || _compiler$options$mod.rules.splice(0, 0, {
      test: this.test,
      use: [{
...
exports.AbstractVanillaExtractPlugin = AbstractVanillaExtractPlugin;
